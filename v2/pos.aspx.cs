...
public partial class v2_pos : System.Web.UI.Page
{
    ...
    public void PrintPage()
    {
        ...
        m_bUseBarcodeInSalesMode = Common.MyBooleanParse(Common.GetSiteSettings("Enable_Barcode_In_Sales_Mode", "0"));
        m_bSetDefaultItemPriceWithGST = Common.MyBooleanParse(Common.GetSiteSettings("Set_Default_Item_Price_With_GST", "1"));
        m_dSetMaximumDiscountPerItem = Common.MyDoubleParse(Common.GetSiteSettings("Set_Maximum_Discount_Per_Item_In_Sales", "30"));

        //sales session control
        if (Request.QueryString["ssid"] != null && Request.QueryString["ssid"] != "")
        {
            m_ssid = Request.QueryString["ssid"];
            if (Session[Common.m_sCompanyName + "sales_dealer_level_for_pos" + m_ssid] == null)
                PrepareNewSales();
        }
        else
        {
        ...
        if (Request.QueryString["gst"] != null && Request.QueryString["gst"] != "")
        {
            ...
            //DEBUG("sc1=",Session[m_sCompanyName + "gst_onoff_for_pos"].ToString());
        ...
        m_sCurrencyName = Common.GetSiteSettings("default_currency_name", "NZD");
        m_dGstRate = Common.MyDoubleParse(Common.GetSiteSettings("gst_rate_percent", "15")) / 100;                        //26.06.2003 NEO	
        ...
        else if (Request.Form["cmd"] == "Set" || (Request.Form["discount_total"] != null && Request.Form["discount_total"] != ""))
        {
            ...
            GetCustomer();
            if (DoApplyDiscountTotal(m_dGstRate, Common.MyBooleanParse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString())))
                Response.Write("<meta http-equiv=\"refresh\" content=\"0; URL=pos.aspx?ssid=" + m_ssid + "\">");
            return;
        }
    ...
    bool RestoreCustomer()
    {
        ...
        m_sSalesType = GetEnumValue("receipt_type", m_quoteType);
        m_bNoIndividualPrice = Common.MyBooleanParse(dr["no_individual_price"].ToString());
        string nip = "0";
    ...
    bool DoSearchItem(string kw, bool btype)
    {
        ...
        sc += " FROM code_relations p Where 1=1 "; // LIKE '"+ kw;
    ...
    void RestoreAllFields()
    {
        ...
        if (Session["sales_no_individual_price" + m_ssid] != null && Session["sales_no_individual_price" + m_ssid].ToString() != "")
            m_bNoIndividualPrice = Common.MyBooleanParse(Session["sales_no_individual_price" + m_ssid].ToString());
    ...
    bool UpdateAllFields()
    {
        ...
        if (Request.Form["nip"] == "on")
            nip = "1";
        m_bNoIndividualPrice = Common.MyBooleanParse(nip);
    ...
    bool PrintCartItemTable()
    {
        ...
        double dRowTotal = 0;
        double dRowSaving = 0;
        bool bAddGSTInPrice = Common.MyBooleanParse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString());
        //build up row list
        for (i = 0; i < dtCart.Rows.Count; i++)
        {
            ...
            double dsupplierPrice = 0; //Common.MyDoubleParse(dr["manual_cost_nzd"].ToString()) * Common.MyDoubleParse(dr["rate"].ToString());
            ...
            try
            {
                ...
                if (Request.Form["del" + i] != "X")
                {
                    ...
                    {
                        sprice = sprice.Replace("$", "");
                        if (Common.MyBooleanParse(Session[m_sCompanyName + "gst_onoff_for_pos"].ToString()))
                            bAddGSTInPrice = false;
                        dSalesPrice = MyCurrencyPrice(sprice);
                        //dSalesPrice = Common.MyDoubleParse(sprice);
                    }
                }
            }
            catch (Exception e)
            ...
            if (m_bOrder)
            {
                SSPrintOneRow(i, dr["code"].ToString(), supplierCode, s_prodName, dsupplierPrice, dSalesPrice, quantity, dRowTotal, dr["s_serialNo"].ToString(), Common.MyDoubleParse(dr["discount_percent"].ToString()), Common.MyBooleanParse(dr["kit"].ToString()));
            ...
            {
                SSPrintOneRow(i, dr["code"].ToString(), dr["code"].ToString(), s_prodName, dsupplierPrice, dSalesPrice, quantity, dRowTotal, dr["s_serialNo"].ToString(), Common.MyDoubleParse(dr["discount_percent"].ToString()), Common.MyBooleanParse(dr["kit"].ToString()));
...
";
        ...
        */
        if (m_customerID != "" && m_customerID != "0")
        {
            ...
            ///////////////////////////////////end here.......///////////////////
            //credit term
            if (Common.MyBooleanParse(Common.GetSiteSettings("enable_credit_term_check", "0")))
            {
                //Response.Write("<tr bgcolor=#EEEEE><td colspan=" + (m_cols-1).ToString() + " align=right>");
                Response.Write("<tr><td  bgcolor=#EEEEE colspan=3 align=right>");
        ...
        if (!Common.MyBooleanParse(Common.GetSiteSettings("discount_include_GST", "0", true)))
    ...
    bool PrintSalesHeaderTable(string sCustomerPONumber)
    {
        ...
        */
        ...
        //DEBUG("sc",Session[m_sCompanyName + "gst_onoff_for_pos"].ToString());
    ...
    void PrintSalesPersonOptions(string current)
    {
        ...
        //if(Session["branch_support"] != null)
        if (int.Parse(Session["employee_access_level"].ToString()) < 8 && Session["branch_support"] != null)
        {
    ...
    bool DoCustomerSearchAndList()
    {
        ...
        sc += " WHERE type <> 3 AND type > 0 and type=1 or type=2 AND main_card_id IS NULL ";
                     ...
                     bool bNoIndividualPrice, ref string order_number)
    {
        ...
        //	sc += " VALUES(0, " + m_quoteType + ", " + Session["card_id"].ToString() + ", '";
        ...
        try
        {
            ...
            if (myCommand1.Fill(dsco, "id") == 1)
            {
                ...
                sc = "UPDATE orders SET number=" + m_orderNumber + ", branch=" + branch_id;
                sc += ", sales_note = N'" + sales_note + "' ";
                if (special_shipto == "1")
                    sc += ", special_shipto=1, shipto = N'" + EncodeQuote(shipto) + "' ";
                sc += ", contact = N'" + contact + "' ";
                if (sales_id != "")
                    sc += ", sales = " + sales_id;
    ...
    {
        if (!Common.MyBooleanParse(Common.GetSiteSettings("enable_bottom_price_check", "1")))
            return true;
        ...
        for (int i = 0; i < dtCart.Rows.Count; i++)
        {
            ...
            if (Common.MyBooleanParse(Common.GetSiteSettings("enable_under_cost_checked", "1", true)))
            {
    ...
    bool DoUpdateOrder()
    {
        ...
        //	sc += ", card_id=" + Session["card_id"].ToString();
        sc += ", freight = " + freight;
        ...
        sc += ", shipto = N'" + EncodeQuote(m_specialShiptoAddr) + "' ";
    ...
    string getCatByCode(string code)
    {
        string sc = "";
        int nRow = 0;
        string Cat = "";
        if (dst.Tables["getCatByCode"] != null)
            dst.Tables["getCatByCode"].Clear();
        sc = " SELECT TOP 1 cat FROM code_relations WHERE 1=1 ";
        sc += " AND code = '" + code + "'";
        try
        {
            SqlDataAdapter ad = new SqlDataAdapter(sc, myConnection);
            nRow = ad.Fill(dst, "getCatByCode");
            if (nRow == 1)
            {
                DataRow dr = dst.Tables["getCatByCode"].Rows[0];
                Cat = dr["cat"].ToString();
                return Cat;
            }
            else
    ...
    bool WriteOrderItems(string order_id)
    {
        ...
        for (int i = 0; i < dtCart.Rows.Count; i++)
        {
            ...
            sc = "INSERT INTO order_item (id, code, quantity, item_name, cat, supplier, supplier_code, supplier_price ";
            sc += ", commit_price, discount_percent) VALUES(" + order_id + ", " + dr["code"].ToString() + ", ";
            sc += dr["quantity"].ToString() + ", N'" + name + "', N'" + getCatByCode(dr["code"].ToString()) + "', N'" + dr["supplier"].ToString();
    ...
    bool ApplyPriceForCustomer()
    {
        ...
        for (int i = 0; i < dtCart.Rows.Count; i++)
        {
            ...
            double dPrice = Common.MyDoubleParse(dr["SalesPrice"].ToString());
