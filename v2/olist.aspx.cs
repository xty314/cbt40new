...
public partial class v2_olist : AdminBasePage
{
    ...
    public void PrintPage()
    {
        ...
        if (Common.MyIntParse(Session[Common.m_sCompanyName + "AccessLevel"].ToString()) == 10)
            m_bAdminLogin = true;

        m_bEnablePrintOverDueStatement = Common.MyBooleanParse(Common.GetSiteSettings("enable_print_overdue_statement", "0", true));
        m_bEnableCreditInvoice = Common.MyBooleanParse(Common.GetSiteSettings("ENABLE_FUNCTION_CREDIT_INVOICE", "0", true));
        ...
        m_bEnableCopyOrderFunction = Common.MyBooleanParse(Common.GetSiteSettings("enable_copy_sales_order_function", "0", true));
        ...
        if (Request.QueryString["o"] != null && Request.QueryString["o"] != "")
            m_nOption = Common.MyIntParse(Request.QueryString["o"]);
        ...
        m_bCanCheck = Common.CheckAccess(Session[Common.m_sCompanyName + "AccessLevel"].ToString(), "checkorder.aspx");
    ...
    string GetIdByEmail(string email)
    {
        string sc = "";
        int rows = 0;
        if (ds.Tables["GetIdByEmail"] != null)
            ds.Tables["GetIdByEmail"].Clear();
        sc = " SELECT id FROM card WHERE emale = '" + email + "' AND type = 4";
        try
        {
            Common.myAdapter = new SqlDataAdapter(sc, Common.myConnection);
            rows = Common.myAdapter.Fill(ds, "GetIdByEmail");
            if (rows == 0)
                return "0";
            else if (rows == 1)
            {
                string id = ds.Tables["GetIdByEmail"].Rows[0]["id"].ToString();
                return id;
            }
        }
        catch (Exception e)
    ...
    Boolean GetOrders()
    {
        ...
        sc += ", o.*, i.paid AS ipaid ";
        sc += ", (select SUM(points * quantity) from order_item where 1=1 and id = o.id) as total_points ";
        sc += " , c2.name, c2.company, c2.trading_name, c.email, c1.name AS lockedby ";
        ...
        sc += " LEFT OUTER JOIN card c2 ON c2.id=i.card_id ";
        if (m_bCTicket)
            sc += " LEFT OUTER JOIN invoice_freight inf ON inf.invoice_number = i.invoice_number OR inf.invoice_number = o.invoice_number ";
        if (m_kw != "")
        {
            if (m_bCTicket)
                sc += " WHERE inf.ticket LIKE '%" + Common.EncodeQuote(m_kw) + "%' ";
            else
            {
                sc += " WHERE (o.po_number LIKE '%" + Common.EncodeQuote(m_kw) + "%' OR o.number LIKE '%" + Common.EncodeQuote(m_kw) + "%' ";
                sc += " OR o.invoice_number LIKE '%" + Common.EncodeQuote(m_kw) + "%' ";
                sc += " OR c.name LIKE '%" + Common.EncodeQuote(m_kw) + "%' OR c.trading_name LIKE '%" + Common.EncodeQuote(m_kw) + "%'";
                sc += " OR o.delivery_number LIKE '%" + Common.EncodeQuote(m_kw) + "%')";
            }
        }
        ...
        {
            sc += " and o.sales = '" + GetIdByEmail(Session["email"].ToString()) + "'";
        }
        if (m_nOption >= 0)
            sc += m_sql[m_nOption];
        if (Request.QueryString["from"] != "" && Request.QueryString["from"] != null)
        ...
        sc += " ORDER BY id DESC";
        //DEBUG("sc +", sc);
        //DEBUG("email", Session["email"].ToString());
        try
        {
            Common.myAdapter = new SqlDataAdapter(sc, Common.myConnection);
            Common.myAdapter.Fill(ds, "order");
            return true;
        }
        catch (Exception e)
        {
    ...
    void BindGrid()
    {
        ...
        {
            Response.Write(" &nbsp; <b>Branch:</b>");
            Common.PrintBranchNameOptions(m_branchID);
            Response.Write("<input type=submit name=cmd value=GO  " + Session["button_style"] + " >");
        }
        ...
        Response.Write("<table class='table table-nowrap'>");
        //Response.Write("<table align=center width='" + tableWidth + "' ");
        //Response.Write(" style=\"font-family:Verdana;font-size:8pt;border-width:1px;border-style:Solid;border-collapse:collapse;fixed\">");
        //Response.Write(">");
        Response.Write("<tr><td><br></td> <td><br></td></tr>");
        ...
        */
        ...
        if (Request.QueryString["p"] != null)
            m_cPI.CurrentPage = Common.MyIntParse(Request.QueryString["p"]);
        ...
        string bgc = Common.GetSiteSettings("table_row_bgcolor", "#EEEEEE");
        string sbgc = Common.GetSiteSettings("sys_quote_bgcolor", "#AAFFFF");
        string sbgcl = Common.GetSiteSettings("sys_quote_bgcolor_light", "#DDFFFF");
        bool bCanChangeInvoice = Common.CheckAccess(Session[Common.m_sCompanyName + "AccessLevel"].ToString(), "invchg.aspx");
        ...
        for (; i < rows && i < end; i++)
        {
            ...
            if (credit_term != "")
                credit_term = Common.GetEnumValue("credit_terms", dr["credit_term"].ToString());
            ...
            if (payment_type != "")
                payment_type = Common.GetEnumValue("payment_method", payment_type);
            ...
            double dstotal = Common.MyDoubleParse(dr["stotal"].ToString());

            bool bIs_redeem = Common.MyBooleanParse(dr["is_redeem"].ToString());
            string sTotalPoints = dr["total_points"].ToString();

            string date = DateTime.Parse(dr["record_date1"].ToString()).ToString("dd-MM-yyyy");
            string shipping_method = Common.GetEnumValue("shipping_method", dr["shipping_method"].ToString());
            shipping_method = shipping_method.ToUpper();
            string p_time = dr["pick_up_time"].ToString();
            string expecting_date = dr["expecting_date"].ToString();
            ...
            else if (shipping_method == "GREEN LAND DELIVERY")
            {
                shipping_method = "<font color=red>GLD ";
                string expecting_sdate = expecting_date;
                if (expecting_sdate != "")
                {
                    shipping_method += "(" + expecting_sdate + ")</font>";
                    sm_title = "Green Land Delivery : " + expecting_date;
                }
            }
            ...
            string status = Common.GetEnumValue("order_item_status", dr["status"].ToString());
            ...
            int days_locked = 0;
            int auto_unlock = Common.MyIntParse(Common.GetSiteSettings("auto_unlock_order_after_days", "2"));
            if (lockedby1 != "")
            {
                ...
                days_locked = Common.MyIntParse(dr["days_locked"].ToString());
            ...
            if (invoice_number != "")
            {
                ...
                //if(Session[Common.m_sCompanyName + "AccessLevel"].ToString() == "10")
                {
                ...
                //			Response.Write(" target=_blank title='click to view invoice without GST' class=o>PLS</a>");
            ...
            if (!bIs_redeem)
                ...
                Response.Write("<td   style=color:" + sStatusIden1 + "><B><font color=red>" + sTotalPoints + "</font></B></td>");
            ...
            if (locker_id != "")
            {
                ...
                //else if(Common.MyIntParse(Session[Common.m_sCompanyName + "AccessLevel"].ToString()) >= 7) //manager and admin can unlock
                else if (Common.bGetAllowAccessID(Session[Common.m_sCompanyName + "AccessLevel"].ToString()))
                {
                    sAction += "<a href=olist.aspx" + BuildParameter() + "&a=u&id=" + id;
                    sAction += " class=o title=Unlock><font color=red>Unlock(admin)</font></a> ";
                }
            }
            if (Common.SecurityCheck("manager", false)
                && (status == "Being Processed" || status == "Back Ordered" || status == "On Hold")
                && Session["access_level"] != Common.GetEnumID("access_level", "Stock man")
                && !bLocked && !bDraft)
            {
                string uri = "pos.aspx?id=" + id;
                if (bSystem)
                    uri = "q.aspx?n=" + id;
                //  int e_olist = int.Parse(Common.GetSiteSettings("edit_order").ToString());
                //int e_olistlevel = int.Parse(Common.GetSiteSettings("allow_to_edit_invoiced_order").ToString());
                //if(e_olist == 1 ){
                //if(int.Parse(Session[Common.m_sCompanyName+"AccessLevel"].ToString()) >=e_olistlevel)
                //{
                if (!bIs_redeem)
    ...
    void PrintOneOrderStatus()
    {
        ...
        */
        ...
        double dGstRate = Common.MyDoubleParse(Common.GetSiteSettings("gst_rate_percent", "12.5")) / 100;             //Modified by NEO
        string gst_rate = ds.Tables["one"].Rows[0]["gst_rate"].ToString();
        if (gst_rate != "")
            dGstRate = Common.MyDoubleParse(gst_rate);
        double dTotal = 0;
        double dTax = 0;
        bool bAlterColor = false;
        //	for(; i < rows && i < end; i++)
        string m_sKitTerm = Common.GetSiteSettings("package_bundle_kit_name", "Kit", true);
        DataRow dr = null;
        for (int i = 0; i < rows; i++)
        {
            ...
            double dAmount = Common.MyDoubleParse(price) * Common.MyIntParse(quantity);
            bool bKit = Common.MyBooleanParse(dr["kit"].ToString());
        ...
        {
            dFreight = Common.MyDoubleParse(dr["freight"].ToString());
            dTotal += dFreight;
        }
    ...
    void OrderOnlyBindGrid()
    {
        ...
        else
        {
            ...
            if (Request.QueryString["t"] == "8")
                ...
                Response.Write(Common.GetEnumValue("order_item_status", Request.QueryString["t"]));
            else
                Response.Write("ALL");
        }
        ...
        for (; i < rows && i < end; i++)
        {
            ...
            else if (shipping_method == "GREEN LAND DELIVERY")
            {
                ...
                {
                    shipping_method += "(" + expecting_sdate + ")</font>";
                    sm_title = "Delivery : " + expecting_date;
                }
            }
            else if (shipping_method == "SUB60")
                shipping_method = "<font color=red>" + shipping_method + "</font>";
    ...
    bool PrintTransferForm()
    {
        ...
        Response.Write(Common.PrintBranchOptions(m_branchID));
    ...
    string GetBranchName(string id)
    {
        ...
        {
            Common.myAdapter = new SqlDataAdapter(sc, Common.myConnection);
            if(Common.myAdapter.Fill(ds, "branch_name") == 1)
    ...
    */
    bool DoTransferOrder()
    {
        ...
        for (int i = 0; i < ds.Tables["one"].Rows.Count; i++)
        {
            ...
            double dQty = Common.MyDoubleParse(qty);
    ...
    bool DoDecreadInvNumber(string inv)
    {
        ...
        string new_nubmer = (Common.MyIntParse(inv) - 1).ToString();
